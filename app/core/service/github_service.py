...
class GithubServiceImpl(GithubService):
    ...
    async def execute_by_type(self, req: Request):
        ...
        body = await req.json()
        provide_type = {
            'ping': self.__deliver_analysed_ping,
            'push': self.__deliver_analysed_push,
            'issue': self.__deliver_analysed_issue,
            'pull_request': self.__deliver_analysed_pr
        }
        try:
    ...
    def __deliver_analysed_ping(self, bot_id: str, body: dict):
    ...
    def __deliver_analysed_pr(self, bot_id: str, body: dict):
    ...
    def __change_variable_which_is_not_valid(self, bot_id: str, body: dict):
        ...
        self.__check_repo_name_is_valid(repo_id, bot_id)
        self.__repo_dao.increase_1_about_event_amt(repo_id)
        self.__repo_dao.update_is_public(repo_id, is_public)

    def __check_repo_name_is_valid(self, repo_id: str, bot_id: str):
